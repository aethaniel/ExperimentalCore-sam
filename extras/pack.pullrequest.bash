...
FILENAME=package_<core>-b${BUILD_NUMBER}.tar.bz2

rm -f $FILENAME

# Change name in platform.txt
sed -i "s/name=.*/name=<core> Pull request #${PR_NUMBER} (Build ${BUILD_NUMBER})/" platform.txt

cd ..
tar --transform "s|$FOLDERNAME|<core>-PR${PR_NUMBER}_b${BUILD_NUMBER}|g"  --exclude=extras/** --exclude=.git* --exclude=.idea -cjf $FILENAME $FOLDERNAME
cd -
